...
echo "🎯 Starting BIRD processes in order..."
echo

# 1. Start core/transit routers (AS 2, 3, 4, 11, 12) first
start_bird_for_pattern 'as[234]r-router|as1[12]r-router' "core/transit routers (AS 2, 3, 4, 11, 12)" 3

# 2. Start route servers at IXes
start_bird_for_pattern 'rs_ix_ix|as10[0-9]rs-' "route servers" 2

# 3. Start stub AS routers (AS 150-171)
start_bird_for_pattern 'as1[567][0-9]r-router' "stub AS routers (AS 150-171)" 0

echo
echo "🎉 All BIRD processes started in order!"
echo "📊 Summary:"
echo "  - Core/transit routers: Started first"
echo "  - Route servers: Started second"  
echo "  - Stub AS routers: Started last"
