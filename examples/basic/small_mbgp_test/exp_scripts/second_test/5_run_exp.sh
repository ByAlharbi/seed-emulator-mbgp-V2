...
#  docker exec -d "$ROUTER149_ID" sh -c "bird -d > /bird/mbgp_log/bird.log 2>&1 &"
  docker exec -d "$ROUTER149_ID" sh -c "bird"
else
...
#  docker exec -d "$ROUTER152_ID" sh -c "bird -d > /bird/mbgp_log/bird.log 2>&1 &"
  docker exec -d "$ROUTER152_ID" sh -c "bird"
else
...
## === Step 3: Start tcpdump on all containers ===
...
## === Step 5: Wait for route propagation ===
echo "⏳ Waiting 10 seconds for route propagation..."
sleep 10

## === Step 6: Determine next experiment number ===
...
# === Step 7: Capture and append route counts ===
