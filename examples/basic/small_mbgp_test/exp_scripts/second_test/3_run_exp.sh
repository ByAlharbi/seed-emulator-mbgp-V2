...
echo -e "\n🚀 Starting BIRD in neighbor routers (149, 151, 152)..."

for rid in "$ROUTER149_ID" "$ROUTER151_ID" "$ROUTER152_ID"; do
  if [[ -n "$rid" ]]; then
...
echo -e "\n🎯 Preparing to profile Router 150..."

# Find unique filename
i=1
while [[ -e "bird_150_profile_${i}.data" ]]; do
  ((i++))
done
PERF_FILE="bird_150_profile_${i}.data"
REPORT_FILE="bird_150_report_${i}.txt"
CPU_LOG="bird_150_cpu_${i}.log"

# === Step 5: Start Router 150 ===
echo "🚀 Starting Router 150..."
docker exec -d "$ROUTER150_ID" sh -c "bird"

# Wait for BIRD to appear
echo "⏳ Waiting for BIRD process..."
for attempt in {1..10}; do
  BIRD_PID=$(docker top "$ROUTER150_ID" 2>/dev/null | grep bird | grep -v grep | awk '{print $2}' | head -1)
  if [[ -n "$BIRD_PID" ]]; then
...
for name in "149:x_as150" "151:x_as150" "152:x_as151"; do
  router=${name%:*}
  proto=${name#*:}
  echo "Router $router:"
