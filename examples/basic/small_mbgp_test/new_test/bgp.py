...
def run(dumpfile=None, hosts_per_as=2): 
    ...
    emu   = Emulator()
    ebgp  = Ebgp()  # Changed from Mbgp to Ebgp
    base  = Base()
    ...
    ###############################################################################
    # Create single-homed stub ASes using standard BGP maker function
    ...
    ###############################################################################
    # BGP Peering via RS (route server) - Changed to Unfiltered
    ...
    # BGP Private peering - Changed all to Unfiltered
    ...
    ebgp.addPrivatePeerings(101, [12], [152, 153], PeerRelationship.Unfiltered)
    ...
    ebgp.addPrivatePeerings(103, [3],  [160, 161, 162], PeerRelationship.Unfiltered)
    ...
    # Note: We're NOT adding Ibgp() and Ospf() layers since we want EBGP only

    emu.addLayer(base)
    emu.addLayer(Routing())
    emu.addLayer(ebgp)  # Changed from mbgp to ebgp
    ...
    if dumpfile is not None: 
        ...
        subprocess.run(["./copy_bird_dir.sh", "./output_bgp"], check=True)
