#!/usr/bin/env python3
# encoding: utf-8
# 20% MBGP deployment - approximately 20% of connections use MBGP
...
def run(dumpfile=None, hosts_per_as=2): 
    ...
    # For 20% MBGP: Only AS150, AS152, AS160 will use MBGP (3 out of 12 ≈ 25%)
    ...
    ###############################################################################
    # RS Peering - Keep most as BGP
    ebgp.addRsPeers(100, [2, 3, 4])
    ebgp.addRsPeers(102, [2, 4])
    ebgp.addRsPeers(104, [3, 4])
    ebgp.addRsPeers(105, [2, 3])
    
    # Private peering - Mix of BGP and MBGP
    # MBGP peerings (20% of connections)
    mbgp.addPrivatePeerings(100, [2], [150], PeerRelationship.Unfiltered)  # MBGP
    mbgp.addPrivatePeerings(101, [12], [152], PeerRelationship.Unfiltered) # MBGP
    mbgp.addPrivatePeerings(103, [3], [160], PeerRelationship.Unfiltered)  # MBGP
    
    # BGP peerings (80% of connections)
    ebgp.addPrivatePeerings(100, [2], [151], PeerRelationship.Unfiltered)
    ebgp.addPrivatePeerings(100, [3], [150], PeerRelationship.Unfiltered)  # AS150 has mixed peers
    
    ebgp.addPrivatePeerings(101, [2], [12], PeerRelationship.Unfiltered)
    ...
    ebgp.addPrivatePeerings(103, [3], [161, 162], PeerRelationship.Unfiltered)
    ...
    if dumpfile is not None: 
        ...
        emu.render()
        emu.compile(Docker(platform=platform), './output_20_mbgp', override=True)
        # Only copy bird directory if using MBGP
        subprocess.run(["./copy_bird_dir.sh", "./output_20_mbgp"], check=True)
