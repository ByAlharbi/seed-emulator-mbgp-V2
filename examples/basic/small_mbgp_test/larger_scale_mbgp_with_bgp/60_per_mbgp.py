#!/usr/bin/env python3
# encoding: utf-8
# 60% MBGP deployment - approximately 60% of connections use MBGP
...
def run(dumpfile=None, hosts_per_as=2): 
    ...
    # For 60% MBGP: AS150, AS151, AS152, AS153, AS160, AS161, AS162, AS163 use MBGP (8 out of 12 ≈ 67%)
    ...
    # Regular BGP stub ASes (only 4 remaining)
    ...
    ###############################################################################
    # RS Peering - Mostly MBGP
    mbgp.addRsPeers(100, [2, 3, 4])  # MBGP RS
    mbgp.addRsPeers(102, [2, 4])     # MBGP RS
    mbgp.addRsPeers(104, [3, 4])     # MBGP RS
    ebgp.addRsPeers(105, [2, 3])     # BGP RS (keep one as BGP)
    
    # Private peering - Mix of BGP and MBGP (60% MBGP)
    # MBGP peerings
    ...
    mbgp.addPrivatePeerings(102, [2, 4], [11], PeerRelationship.Unfiltered)
    ...
    # BGP peerings (40%)
    ebgp.addPrivatePeerings(102, [2, 4], [154], PeerRelationship.Unfiltered)
    ...
    if dumpfile is not None: 
        ...
        emu.render()
        emu.compile(Docker(platform=platform), './output_60_mbgp', override=True)
        # Only copy bird directory if using MBGP
        subprocess.run(["./copy_bird_dir.sh", "./output_60_mbgp"], check=True)
