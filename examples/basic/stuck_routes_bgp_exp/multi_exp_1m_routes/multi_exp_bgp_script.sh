...
SHUTDOWN_OFFSET=25  # Seconds after route configuration to trigger 149 shutdown
EXPERIMENT_DURATION=50  # Seconds to wait after 149 shutoff before ending experiment
CHECK_STUCK_DELAY=3  # Seconds to wait after killing 149 before checking for stuck routes
...
run_first_experiment() {
    ...
    # 12. Wait briefly then check for stuck routes
    echo -e "${YELLOW}⏱️ Waiting ${CHECK_STUCK_DELAY} seconds before checking for stuck routes...${NC}"
    sleep $CHECK_STUCK_DELAY
    check_stuck_routes 1
    
    # 13. Let the experiment run for the specified duration after 149 shutoff
