...
# === Step 5: Start BIRD in routers in specific order ===
ordered_routers=(
  "as151r-router0-10.151.0.254"
  "as152r-router0-10.152.0.254"
)
echo "🚀 Starting BIRD in routers in order: 152 → 151 → 150 → 149"
for name in "${ordered_routers[@]}"; do
  container_id=$(docker ps -qf "name=$name")
  if [[ -n "$container_id" ]]; then
    echo "-- Starting BIRD in $name"
